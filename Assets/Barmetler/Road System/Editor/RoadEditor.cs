...
namespace Barmetler.RoadSystem
{
    ...
    public class RoadEditor : Editor
    {
        ...
        void DrawInfo(int controlID)
        {
            ...
            var points = road.GetEvenlySpacedPoints(1, 1)
                .Select(e => e.ToWorldSpace(road.transform))
                .ToArray();
        ...
        void GUIControlPoints(int controlID)
        {
            ...
            switch (Tools.current)
            {
                ...
                case Tool.Scale:
                    ...
                    var points = Enumerable
                        ...
                        .Select(
                            e => (index: e * 3, pos: road.transform.TransformPoint(road[e * 3]))
                        )
                        .OrderByDescending(
                            e =>
                                Vector3.Dot(
                                    Camera.current.transform.forward,
                                    e.pos - Camera.current.transform.position
                                )
                        )
                        .ToList();
                        ...
                        if (selectedAnchorPoint != p.index)
                        {
                            if (hasModifiers)
                                Handles.SphereHandleCap(
                                    ...
                                    p.pos,
                                    Quaternion.identity,
                                    0.2f * HandleUtility.GetHandleSize(p.pos),
                                    EventType.Repaint
                                );
                            else if (
                                Handles.Button(
            ...
            if (selectedAnchorPoint != -1)
            {
                ...
                rot = RoadUtilities.GetRotationAtWorldSpace(
                    road,
                    selectedAnchorPoint,
                    out forward,
                    out _
            ...
            if (!e.control)
            {
                switch (Tools.current)
                {
                    case Tool.Move:
                        if (selectedAnchorPoint != -1)
                        {
                            ...
                            if (newPos != pos)
                            {
                                ...
                                road.MovePoint(
                                    selectedAnchorPoint,
                                    road.transform.InverseTransformPoint(newPos)
                                );
                            }
                        }
                        break;
                    ...
                    case Tool.Rotate:
                        if (selectedAnchorPoint != -1)
                        {
                            ...
                            )
                            {
                                ...
                                RoadUtilities.SetRotationAtWorldSpace(
                                    road,
                                    selectedAnchorPoint,
                                    newRot
                                );
                    ...
                    case Tool.Scale:
                        if (selectedAnchorPoint != -1)
                        {
                            ...
                            Handles.SphereHandleCap(
                                ...
                                pos,
                                Quaternion.identity,
                                (hasModifiers ? 0.2f : 0.3f) * HandleUtility.GetHandleSize(pos),
                                EventType.Repaint
                            );
                            ...
                            for (int i = -1; i <= 1; i += 2)
                            {
                                ...
                                if (hasModifiers)
                                {
                                    Handles.SphereHandleCap(
                                        ...
                                        hPos,
                                        Quaternion.identity,
                                        0.2f * HandleUtility.GetHandleSize(pos),
                                        EventType.Repaint
                                    );
                                }
                                else
                                {
                                    var fmh_246_50_638277445636665498 = Quaternion.identity;
                                    var nPos = Handles.FreeMoveHandle(
                                        hPos,
                                    ...
                                    if (hPos != nPos)
                                    {
                                        ...
                                        road.MovePoint(
                                            j,
                                            road.transform.InverseTransformPoint(
                                                pos + forward * dot
                                            )
                                        );
                                    }
                    ...
                    case Tool.Rect:
                        ...
                        {
                            var c =
                                ((p.index + 1) / 3 * 3) == selectedAnchorPoint
                            ...
                            {
                                var fmh_298_87_638277445636731249 = Quaternion.identity;
                                var newPos = Handles.FreeMoveHandle(
                                    $"Handle-{p.index}".GetHashCode(),
                                    p.pos,
                                    (p.index % 3 == 0 ? 0.3f : 0.25f)
                                        * HandleUtility.GetHandleSize(p.pos),
                                    Vector3.zero,
                                    Handles.SphereHandleCap
                                );
        ...
        void GUIAddOrRemovePoints(int controlID)
        {
            ...
            if (e.shift && !e.alt && !e.control)
            {
                ...
                for (int seg = 0; seg < road.NumSegments; ++seg)
                {
                    var v = Bezier
                        .GetEvenlySpacedPoints(
                            ...
                            new List<Vector3> { road.GetNormal(seg), road.GetNormal(seg + 1) },
                            1
                        )
                        .Select(e => e.ToWorldSpace(road.transform).position)
                        .ToArray();
                ...
                if (segmentIndex != -1)
                {
                    ...
                    if (e.type == EventType.MouseDown && e.button == 0)
                    {
                        ...
                        road.InsertSegment(
                            road.transform.InverseTransformPoint(hoverPos),
                            segmentIndex
                        );
                    }
                }
            ...
            if (e.control && !e.alt)
            {
                ...
                if (selectedEndpoint.isValid)
                {
                    ...
                    RoadUtilities.GetRotationAtWorldSpace(
                        road,
                    ...
                    if (e.shift && new Plane(-up, d).Raycast(ray, out var enter) && enter < 500)
                    {
                        ...
                        RSHandleUtility.DrawGridCircles(
                            selectedEndpoint.position,
                            right,
                            forward,
                            5,
                            new[] { (selectedEndpoint.position, 20.0f), (position, 20.0f), }
                        );
                        Handles.color = c;
                    }
                    else if (
                        !e.shift && settings.UseRayCast && Physics.Raycast(ray, out var rayHit, 500)
                    )
                    {
                    ...
                    else
                    {
                        var direction =
                            ray.direction
                            / Vector3.Dot(Camera.current.transform.forward, ray.direction);
                    ...
                    Handles.DrawLine(
                        position,
                        position
                            + 0.3f * HandleUtility.GetHandleSize(selectedEndpoint.position) * up
                    );
                    Handles.ArrowHandleCap(
                        ...
                        1 * HandleUtility.GetHandleSize(selectedEndpoint.position),
                        EventType.Repaint
                    );
                    ...
                    if (e.type == EventType.MouseDown && e.button == 0)
                    {
                        ...
                        road.AppendSegment(
                            road.transform.InverseTransformPoint(position),
                            selectedEndpoint.isStart,
        ...
        public bool Extrude(ref int i)
        {
            if (i == 0 || i == road.NumPoints - 1)
            {
                if ((i == 0 && road.start == null) || (i != 0 && road.end == null))
                {
                    ...
                    road.AppendSegment(
                        road[endIndex] - (road[controlIndex] - road[endIndex]).normalized * 2,
                        i == 0
                    );
                    if (i != 0)
                        i = road.NumPoints - 1;
                ...
                (int WindowID) =>
                {
                    if (selectedAnchorPoint != -1)
                    {
                        ...
                        if (newVec != oldVec)
                        {
                            ...
                            road.MovePoint(
                                selectedAnchorPoint,
                                road.transform.InverseTransformPoint(newVec)
                            );
                        }

                        oldVec = RoadUtilities
                            .GetRotationAtWorldSpace(road, selectedAnchorPoint)
                            .eulerAngles;
        ...
        public override void OnInspectorGUI()
        {
            ...
            BoolField(
                ...
                road.AutoSetControlPoints,
                v => road.AutoSetControlPoints = v,
                road,
                false
            );
